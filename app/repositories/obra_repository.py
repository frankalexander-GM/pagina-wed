...
class ObraRepository(BaseRepository):
    ...
    def get_total_favoritos_artista(self, artista_id):
        """
        Obtener total de favoritos de todas las obras de un artista
        
        Args:
            artista_id (int): ID del artista
            
        Returns:
            int: Total de favoritos
        """
        try:
            from sqlalchemy import func
            
            result = self.session.query(
                func.count(favoritos_obras.c.id_usuario)
            ).join(
                Obra, Obra.id_obra == favoritos_obras.c.id_obra
            ).filter(
                Obra.id_artista == artista_id
            ).scalar()
            
            return result or 0
        except SQLAlchemyError as e:
            print(f"Error al obtener favoritos del artista: {e}")
            return 0
    ...
    def get_favoritos_count(self, usuario_id):
        ...
        try:
            ...
            print(f"Error al contar favoritos: {e}")
            return 0
    ...
    def get_obras_por_categoria(self, categoria_id, limit=None, offset=None):
        ...
        Returns:
            list: Lista de obras de la categoría
        """
        return self.get_by_categoria(categoria_id, limit=limit, offset=offset)
